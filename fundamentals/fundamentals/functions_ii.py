# Countdown - Create a function that accepts a number as an input. Return a new list that counts down by one, 
# from the number (as the 0th element) down to 0 (as the last element).
# Example: countdown(5) should return [5,4,3,2,1,0]
def countDownByOne(a):
    newList = []
    for x in range(a, 0, -1):
        newList.append(x)
    return newList
testNum = 9
answerList = countDownByOne(testNum)
print(answerList)
# Print and Return - Create a function that will receive a list with two numbers. Print the first value and return the second.
# Example: print_and_return([1,2]) should print 1 and return 2
def print0Return1(someList):
    print(someList[0])
    return someList[1]
testList = [3,4]
returnValue = print0Return1(testList)
print(returnValue)

# First Plus Length - Create a function that accepts a list and returns the sum 
# of the first value in the list plus the list's length.
# Example: first_plus_length([1,2,3,4,5]) should return 6 
# (first value: 1 + length: 5)
def firstPlusLength(someList):
    return someList[0] + len(someList)
testList2 = [1,2,3,4,5]
returnValue2 = firstPlusLength(testList2)
print(returnValue2)

# Values Greater than Second - Write a function that accepts a list and 
# creates a new list containing only the values from the original list 
# that are greater than its 2nd value. Print how many values this is and 
# then return the new list. If the list has less than 2 elements, 
# have the function return False
# Example: values_greater_than_second([5,2,3,2,1,4]) print 3 and return [5,3,4]
# Example: values_greater_than_second([3]) should return False
def valuesGreaterThanSecond(someList):
    if(len(someList) < 2):
        return False
    else:
        newList = []
        for i in range(0, len(someList), 1):
            if(someList[i] > someList[1]):
                newList.append(someList[i])
    return newList
test1 = [5,2,3,2,1,4]
answer1 = valuesGreaterThanSecond(test1)
print(answer1)
test2 = [3]
answer2 = valuesGreaterThanSecond(test2)
print(answer2)

# This Length, That Value - Write a function that accepts two integers as parameters
# : size and value. The function should create and return a list whose length is 
# equal to the given size, and whose values are all the given value.
# Example: length_and_value(4,7) should return [7,7,7,7]
# Example: length_and_value(6,2) should return [2,2,2,2,2,2]
def thisLengthThatValue(x, y):
    someList = []
    for x in range(0, x, 1):
        someList.append(y)
    return someList
tester1 = thisLengthThatValue(4,7)
print(tester1)
tester2 = thisLengthThatValue(6,2)
print(tester2)
